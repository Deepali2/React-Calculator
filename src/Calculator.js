...
class Calculator extends Component {
  ...
  numberButtonClick = e => {
    ...
    if (this.state.resultDisplayed === false) {
      ...
      this.state.resultDisplayed === true &&
      operationChars.indexOf(lastChar) !== -1
    ) {
      currentString += e.target.value;
      this.setState({
        resultDisplayed: false,
  ...
  equalsPressed = () => {
    ...
    for (let i = 0; i < operationChars.length; i++) {
      ...
      while (indexOfOperationToExecute !== -1) {
        let currentResult = currentOperationFunction(
          numbers[indexOfOperationToExecute],
          numbers[indexOfOperationToExecute + 1]
        );
